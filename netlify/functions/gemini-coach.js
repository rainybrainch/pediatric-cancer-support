...
exports.handler = async (event) => {
  ...
  if (!userText) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'userText is required' }) };
  }
  if (userText.length > 2000) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'userText too long (max 2000)' }) };
  }
